...
class Deriva(Globus):
    def __init__(self, redirectUri, clientId=None, clientSecret=None):
        ...
        self.__AUTH_SCOPES.append("my_scope")


    def getClientIdSetting(self):
        return Setting().get(constants.PluginSettings.DERIVA_CLIENT_ID)

    def getClientSecretSetting(self):
        return Setting().get(constants.PluginSettings.DERIVA_CLIENT_SECRET)

    @classmethod
    def getUrl(cls, state):
        clientId = Setting().get(constants.PluginSettings.DERIVA_CLIENT_ID)

        if clientId is None:
            raise Exception('No Deriva client ID setting is present.')

        callbackUrl = '/'.join((getApiUrl(), 'oauth', 'deriva', 'callback'))
